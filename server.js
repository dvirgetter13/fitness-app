...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (סופי, חסין ומעוצב) ---
...
- מאכלים/רגישויות אסורים: ${cleanDisliked || 'אין'}
...
חוקי כמויות (התאמה ל-${calories} קלוריות):
- השתמש בכמויות נדיבות, משביעות וריאליות בצלחת (למשל: 200-250 גרם חזה עוף/פרגית/בקר/טופו, 250-300 גרם אורז/פסטה/תפו"א מבושלים, 3-4 פרוסות לחם מלא, 20-30 גרם שקדים/אגוזים/שמן זית/אבוקדו).

חוקי איסור וסינון:
- אסור להשתמש באף מאכל המופיע ברשימה: "${cleanDisliked}".
- אם צוין "חלב" - אסור להשתמש ביוגורט, גבינות, קוטג' ומעדנים (השתמש בטופו, טונה, עוף, אגוזים, לחם).
- אם צוין "טחינה" - אסור להשתמש בטחינה (השתמש בשמן זית, אבוקדו, שקדים).
- אם צוין "ביצים" - אסור להשתמש בביצים/חביתה.
...
💡 טיפ זהב: [טיפ קצר להתמדה]
    ...
    try {
        ...
        // 🧹 ניקוי סופי ב-Backend למניעת תקלות עיצוב ושרידי סוגריים
        menuText = menuText.replace(/\*/g, '');
        menuText = menuText.replace(/#/g, '');
        menuText = menuText.replace(/\(.*?\)/g, ''); // מחיקת כל מה שבתוך סוגריים אם נוצרו
        menuText = menuText.replace(/[\(\)]/g, ''); // הסרת סוגריים יתומים
