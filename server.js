...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (משודרג, טעים וישראלי) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
אתה דיאטן ספורט קליני בכיר בישראל. בנה תפריט תזונה יומי מותאם אישית, טעים, הגיוני וריאלי.

נתוני המשתמש:
- יעד: ${goal === 'muscle' ? 'בניית מסת שריר' : goal === 'fat_loss' ? 'חיטוב וירידה באחוז שומן' : 'שמירה על המשקל'}
- משקל: ${weight ? weight + ' ק"ג' : 'לא צוין'}
- יעד קלורי יומי: ${calories} קלוריות (חובה להגיע בסה"כ ל-${calories} קק"ל בקירוב של ±50 קלוריות!)
- יעד חלבון יומי: ${protein} גרם חלבון
- מאכלים שאינו אוהב/נמנע מהם/אלרגיות: ${dislikedFoods || 'אין'}

כללים נוקשים לבניית התפריט:
1. התפריט חייב להיות הגיוני ומותאם למטבח הישראלי:
   - ארוחת בוקר: לחם מלא/קל, ביצים/חביתה, גבינה לבנה/קוטג'/צהובה 9%, ירקות, או דגני שיבולת שועל עם חלב/יוגורט.
   - ארוחת צהריים: מנה בשרית (חזה עוף/פרגית/בקר רזה/דג) + פחמימה עיקרית אחת בלבד (אורז/תפוח אדמה/פסטה/קוסקוס/פתיתים) + סלט ירקות וכלול שומן בריא (כמו כפית שמן זית/טחינה).
   - ארוחת ביניים: יוגורט חלבון / פרי / שקדים ואגוזים / סקופ אבקת חלבון.
   - ארוחת ערב: ביצים/טונה/גבינות + לחם מלא + אבוקדו/טחינה + סלט ירקות.
2. איסור מוחלט על שילובים מוזרים (למשל: אל תשים שיבולת שועל יחד עם אורז/קוסקוס בצהריים, אל תשים אורז בבוקר!).
3. השתמש בשמות עבריים תקניים בלבד (למשל: "שקדים" ולא "אלמונד", "אגוזי מלך" ולא "מעונט", "שיבולת שועל" ולא "אובלים").
4. חשב במדויק את סך הקלוריות והחלבון היומי כך שהחיבור של כל 4 הארוחות יגיע במדויק ל-${calories} קלוריות ו-${protein} גרם חלבון.
5. פרט לכל ארוחה: רכיבים, כמויות מדויקות (בגרמים/כפות/פרוסות/יחידות), קלוריות וחלבון.
6. הוסף סיכום סופי קצר, מדויק ונקי, וטיפ זהב קצר להצלחה.
7. הצג ישירות את התפריט ללא פתיחים וללא אזכור של AI.
    `;
        ...
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'אתה מנוע תזונה קלינית וספורטיבית מתקדם. תפקידך להפיק תפריטי תזונה מפורטים, מובנים, מדויקים מבחינה קלורית ומקצועיים בלבד בשפה העברית.'
                    },
                    {
                        role: 'user',
                        content: prompt
