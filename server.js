...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (אלגוריתם חסין מקרי קצה) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
אתה מנוע תזונה קלינית וספורטיבית בכיר. תפקידך לבנות תפריט יומי יחיד, מדויק, טעים והגיוני לחלוטין.

נתוני המשתמש:
- יעד: ${goal === 'muscle' ? 'בניית מסת שריר' : goal === 'fat_loss' ? 'חיטוב וירידה באחוז שומן' : 'שמירה על המשקל'}
- יעד קלורי יומי: ${calories} קלוריות (חובה להגיע לסכום זה בדיוק על ידי התאמת גרמים בצלחת!)
- יעד חלבון יומי: ${protein} גרם חלבון
- רשימת מאכלים ואלרגיות שאסור להשתמש בהם בכלל: ${dislikedFoods || 'אין'}

חוקי ברזל להרכבת המנות (איסור מוחלט על ערבובים מוזרים!):
1. ארוחת בוקר: פחמימת בוקר (לחם מלא/קל, שיבולת שועל) + חלבון מתאים (יוגורט/גבינות/טונה/טופו) + שומן (אגוזים/שקדים/אבוקדו). איסור מוחלט על אורז/פתיתים/בשר בבוקר!
2. ארוחת צהריים: חלבון עיקרי (חזה עוף/פרגית/בקר רזה/הודו/טופו) + פחמימה חמה (אורז/תפו"א/פסטה/קוסקוס/פתיתים) + ירקות ושמן זית. איסור מוחלט על מוצרי חלב, שיבולת שועל או מתוקים בצהריים!
3. ארוחת ביניים: יוגורט חלבון / מעדן פרו / פרי / שקדים / אגוזי מלך.
4. ארוחת ערב: חלבון (טונה/גבינות/ביצים/בקר/עוף) + פחמימה (לחם מלא/תפו"א) + שומן בריא (טחינה/אבוקדו) + ירקות.

חוקי ערכים תזונתיים ועיצוב:
1. ערכים אמיתיים בלבד: 100 גרם עוף/בקר/טונה = ~25-30 גרם חלבון. 100 גרם אורז מבושל = ~120 קלוריות. אל תמציא ערכים מופרכים (כמו 1000 קלוריות על מנה קטנה)!
2. כדי להגיע ל-${calories} קלוריות, הגדל את הפיזיות בצלחת (למשל 250 גרם עוף, 250 גרם אורז, 3 פרוסות לחם, 30 גרם שקדים).
3. ללא תגיות Markdown (אל תשתמש ב-**, #, -).
4. ללא תפריטים כפולים, ללא אלטרנטיבות, וללא סעיפי תיקונים/הפרשים.
5. השתמש בשמות עבריים תקניים בלבד.

פורמט יציאה קשיח ללא סטיות:
🍳 ארוחת בוקר: [רכיבים וכמויות] - [קלוריות] קלוריות, [חלבון] גרם חלבון
🥗 ארוחת צהריים: [רכיבים וכמויות] - [קלוריות] קלוריות, [חלבון] גרם חלבון
🏋️‍♂️ ארוחת ביניים: [רכיבים וכמויות] - [קלוריות] קלוריות, [חלבון] גרם חלבון
🍴 ארוחת ערב: [רכיבים וכמויות] - [קלוריות] קלוריות, [חלבון] גרם חלבון
📊 סיכום: ${calories} קלוריות, ${protein} גרם חלבון.
💡 טיפ זהב: [טיפ קצר להתמדה]
    `;
    ...
    try {
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'אתה מנוע תזונה קלינית וספורטיבית מתקדם. תפקידך להפיק תפריט תזונה יחיד, הגיוני, אסתטי ומדויק לחלוטין בשפה העברית.'
                    },
                    {
                        role: 'user',
                        content: prompt
        ...
        // --- 🧹 ניקוי וסינון Backend אוטומטי למקרי קצה ---
        menuText = menuText.replace(/\*/g, ''); // הסרת כוכביות
        menuText = menuText.replace(/#/g, '');  // הסרת סולמיות
        menuText = menuText.replace(/קווייקר אובל|אובל/g, 'שיבולת שועל'); // תיקון שגיאת אובל
        menuText = menuText.replace(/אגוזי אלה/g, 'אגוזי לוז'); // תיקון שגיאת אגוזי אלה
        menuText = menuText.replace(/אלמונד/g, 'שקדים'); // תיקון שגיאת אלמונד
        menuText = menuText.replace(/(טיפ:|💡 טיפ)/g, '\n💡 טיפ'); // הבטחת ירידת שורה לטיפ

        // אם המודל ייצר בטעות תפריט כפול, נחתוך רק את התפריט הראשון עד הטיפ הראשון
        if (menuText.split('💡 טיפ').length > 2) {
            const parts = menuText.split('💡 טיפ');
            menuText = parts[0] + '💡 טיפ' + parts[1];
        }
