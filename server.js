...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (אלגוריתם חסין ומדויק) ---
...
- רשימת מאכלים/רגישויות שאסור להשתמש בהם כלל: ${dislikedFoods || 'אין'}

כללי איסור וסינון חמורים (חובה!):
1. עבור על כל המילים שצוינו ברשימת המאכלים הבלתי רצויים (${dislikedFoods || 'אין'}). אסור באיסור מוחלט להשתמש באף אחד מהם!
2. אם צוין "חלב" או "מוצרי חלב" - אסור להשתמש ביוגורט, גבינות, קוטג' או מעדנים.
3. אם צוין "טחינה" - אסור להשתמש בטחינה (השתמש במקום בשמן זית, אבוקדו, שקדים או אגוזים).
4. אם צוין "ביצים" - אסור להשתמש בביצים/חביתה.

כללי ערכים תזונתיים וכמויות חובה (הגדל את הגרמים בצלחת!):
- 1 פרוסת לחם מלא = 70 קלוריות, 4ג חלבון
- 100ג חזה עוף / פרגית / טונה / בשר / דג = 140 קלוריות, 25ג חלבון (עבור יעד של כ-1900 קלוריות השתמש ב-200 גרם בארוחה!).
- 100ג אורז / תפו"א / פסטה מבושלים = 120 קלוריות, 2.5ג חלבון (השתמש ב-200 גרם בארוחה!).
- 10ג שמן זית / טחינה / אגוזים / שקדים = 70 קלוריות
- 1 יוגורט חלבון (200ג) = 140 קלוריות, 20ג חלבון

הנחיה חורצת:
ודא שסכום הקלוריות הרשום ב-4 הארוחות בנפרד מסתכם בדיוק ל-${calories} קלוריות ול-${protein} גרם חלבון!
איסור מוחלט לכתוב "חסר קלוריות", "נוסיף" או "תיקון" בתחתית התפריט.

חוקי הרכבת המנות:
1. ארוחת בוקר: לחם מלא/שיבולת שועל + חלבון מורשה (יוגורט/גבינות/טונה/אגוזים אם מותר). אל תשים אורז בבוקר!
2. ארוחת צהריים: חזה עוף/פרגית/בקר + אורז/תפו"א/פסטה + ירקות ושמן זית. אל תשים יוגורט/שיבולת שועל בצהריים!
3. ארוחת ביניים: יוגורט חלבון / מעדן פרו / פרי + שקדים או אגוזים (בהתאם לסינון).
4. ארוחת ערב: טונה/גבינות/ביצים/בקר/עוף + לחם מלא/תפו"א + שומן בריא מורשה + ירקות.

פורמט יציאה קשיח (התחל ישר מ-🍳 ארוחת בוקר, ללא פתיחים וללא שורות תיקון בסוף):
...
💡 טיפ זהב: [טיפ קצר להתמדה]
    ...
    try {
        ...
        menuText = menuText.replace(/אלמונד/g, 'שקדים');
