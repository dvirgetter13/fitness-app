...
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
בנה תפריט תזונה יומי יחיד, נקי, מעוצב ומותאם אישית בלבד.

נתוני המשתמש:
- יעד: ${goal === 'muscle' ? 'בניית מסת שריר' : goal === 'fat_loss' ? 'חיטוב וירידה באחוז שומן' : 'שמירה על המשקל'}
- יעד קלוריות יומי: ${calories} קלוריות
- יעד חלבון יומי: ${protein} גרם חלבון
- מאכלים/רגישויות/העדפות (כגון צמחוני/טבעוני/ללא פחמימות): ${cleanDisliked || 'אין'}

חוקי הרכבת ארוחות הגיוניות וטבעיות:
1. ארוחת בוקר: אך ורק ביצים / חביתה / גבינות / יוגורט / מעדן חלבון / טופו (אם מותר) + ירקות, אגוזים ושומן בריא. איסור מוחלט לשים עוף, בשר, דגים, פרגיות או סלמון בארוחת בוקר! אם המשתמש ביקש ללא פחמימות, השתמש בביצים/גבינות/יוגורט/אבוקדו/אגוזים ולא בלחם/שיבולת שועל.
2. ארוחת צהריים: חלבון מרכזי (עוף/בשר/דג/טופו/סייטן) + פחמימה חמה (אם מותר) + ירקות ושמן זית.
3. ארוחת ביניים: יוגורט/מעדן/אגוזים/שקדים/פרי (אם מותר).
4. ארוחת ערב: חלבון (ביצים/גבינות/טונה/בשר/טופו) + ירקות ושומן בריא.

חוקי פורמט ועיצוב קשיחים:
1. אל תרשום קלוריות, חלבון או חישובים לצד הארוחות הבודדות! פרט אך ורק את הרכיבים והכמויות המדויקות.
2. איסור מוחלט להשתמש בסוגריים () בשום מקום בתפריט!
3. איסור מוחלט לרשום הודעות תיקון, חישובים פנימיים או פסקאות נוספות.
4. אל תשתמש ב-Markdown (ללא **, #, -).

חוקי איסור וסינון:
- אם צוין "צמחוני" - אסור להשתמש בבשר, עוף, פרגית, הודו או דגים!
- אם צוין "ללא פחמימות" או "אסור פחמימות" - אסור להשתמש בלחם, אורז, פסטה, תפוחי אדמה, שיבולת שועל, קראנצ', קוואקר ופירות.
- אם צוין "חלב" - אסור להשתמש ביוגורט, גבינות, קוטג' ומעדנים.

פורמט יציאה קשיח שחובה לשכפל בדיוק:
🍳 ארוחת בוקר: [רכיבים וכמויות מפורטות]
🥗 ארוחת צהריים: [רכיבים וכמויות מפורטות]
🏋️‍♂️ ארוחת ביניים: [רכיבים וכמויות מפורטות]
🍴 ארוחת ערב: [רכיבים וכמויות מפורטות]
        ...
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'אתה מנוע תזונה קלינית וספורטיבית מתקדם. תפקידך להפיק תפריט יחיד ונקי המפרט רכיבים וכמויות בלבד ללא סוגריים, ללא קלוריות בכל ארוחה וללא חישובים.'
                    },
                    {
                        role: 'user',
                        content: prompt
