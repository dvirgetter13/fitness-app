...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (תפריט יחיד, נקי ומדויק) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
אתה דיאטן ספורט קליני בכיר. בנה תפריט תזונה יומי מותאם אישית אחד בלבד! 
...
1. בנה תפריט אחד בלבד עבור יום אחד! אל תייצר מספר תפריטים ואל תציע אלטרנטיבות.
2. אל תשתמש במילה "אובל" או "קווייקר אובל"! השתמש אך ורק במילה "שיבולת שועל". השתמש בשמות עבריים תקניים בלבד.
3. ללא תגיות Markdown (אל תשתמש ב-**, #, -).
4. ללא סעיפי "תיקונים", "הערות", או "הצעות לשינוי". הצג רק את 4 הארוחות וסיכום סופי!
5. השתמש באייקונים בתחילת כל שורה.

מבנה מדויק שחקוק בסלע:
...
💡 טיפ זהב: [טיפ קצר]
    ...
    try {
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'אתה מנוע תזונה קלינית וספורטיבית מתקדם. תפקידך להפיק תפריטי תזונה מובנים, אסתטיים, נקיים לחלוטין ללא תגיות Markdown, ללא תפריטים כפולים וללא סעיפי תיקון.'
                    },
                    {
                        role: 'user',
                        content: prompt
        ...
        // ניקוי כוכביות, מילים שגויות וסידור ירידת שורה
