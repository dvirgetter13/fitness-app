...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (דיוק מתמטי + ויזואליות נקייה) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
אתה דיאטן ספורט קליני בכיר. בנה תפריט תזונה יומי מותאם אישית, נקי, ויזואלי ומדויק מבחינה מתמטית.
...
כללים קשיחים:
1. ללא תגיות Markdown (אל תשתמש כלל ב-**, #, -).
2. השתמש באייקונים בתחילת כל שורה.
3. הקפד על ערכים קלוריים ריאליים ופרופורציונליים! (ארוחה קלה של ביצים ולחם היא כ-350-450 קלוריות, אל תרשום עליה 800+ קלוריות).
4. כדי להגיע ליעד הקלורי של ${calories} קלוריות, הגדל את הכמויות בצלחת (למשל: 200 גרם חזה עוף, 200-250 גרם אורז, 3 פרוסות לחם, הוספת כף שמן זית/טחינה/אגוזים) ולא סתם לרשום מספר קלוריות שאינו תואם את המזון.

דוגמה למבנה נכון:
🍳 ארוחת בוקר: 2 ביצים קשות, 2 פרוסות לחם מלא, 30 גרם גבינה בולגרית 5%, מלפפון ועגבניה - 350 קלוריות, 22 גרם חלבון
🥗 ארוחת צהריים: 200 גרם חזה עוף צלוי, 200 גרם אורז מבושל, סלט ירקות עם כף שמן זית - 650 קלוריות, 50 גרם חלבון
🏋️‍♂️ ארוחת ביניים: 200 גרם יוגורט פרו, תפוח, 20 גרם שקדים - 280 קלוריות, 25 גרם חלבון
🍴 ארוחת ערב: קופסת טונה במים, 2 פרוסות לחם מלא, 1/2 אבוקדו, סלט ירקות - 450 קלוריות, 35 גרם חלבון
💡 סיכום: 1730 קלוריות, 132 גרם חלבון. טיפ: הקפד על שתיית מים מרובה.
    `;
    ...
    try {
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'אתה מנוע תזונה קלינית וספורטיבית מתקדם. תפקידך להפיק תפריטי תזונה מובנים, אסתטיים, נקיים לחלוטין ללא תגיות Markdown ומדויקים ערכית.'
                    },
                    {
                        role: 'user',
                        content: prompt
        ...
        // ניקוי שאריות כוכביות אם המודל ייצר כאלה
