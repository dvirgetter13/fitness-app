...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (משודרג ומדויק) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
אתה דיאטן ספורט מומחה. בנה תפריט תזונה יומי מותאם אישית.

נתוני המשתמש:
- יעד תזונתי: ${goal === 'muscle' ? 'בניית מסת שריר' : goal === 'fat_loss' ? 'חיטוב וירידה באחוז שומן' : 'שמירה על המשקל'}
- משקל: ${weight ? weight + ' ק"ג' : 'לא צוין'}
- יעד קלורי יומי מדויק: ${calories} קלוריות (חובה להגיע לסך קלוריות זה בקירוב של ±50 קלוריות!)
- יעד חלבון יומי מדויק: ${protein} גרם חלבון
- מאכלים/רכיבים שהמשתמש אינו אוהב/אלרגי אליהם: ${dislikedFoods || 'אין רגישויות'}

דרישות חובה מדויקות:
1. סך הקלוריות וסך החלבון בכל 4 הארוחות יחד חייבים להיות שווים ליעד היומי (${calories} קלוריות ו-${protein} גרם חלבון)! התאם את הכמויות בגרמים של האורז, הלחם, הבשר, והשומנים הבריאים (כמו שמן זית, אגוזים, טחינה, אבוקדו) כדי להגיע בול ליעד.
2. הקפד על שמות מאכלים נכונים ומדויקים בעברית (למשל: שיבולת שועל ולא "אובלים").
3. אל תשתמש כלל במאכלים שצוינו ברשימת המאכלים הלא רצויים/אלרגיות.
4. פרט עבור כל ארוחה: רכיבים, כמויות מדויקות (בגרמים/כפות/יחידות), קלוריות וחלבון.
5. הוסף סיכום סופי מדויק של סך הקלוריות והחלבון בתפריט, וטיפ קצר להצלחה.
6. אל תציין מילות קישור, פתיחים או אזכור של AI.
    `;
