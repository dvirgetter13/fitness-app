...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (DETERMINISTIC CLEAN FORMAT) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
בנה תפריט תזונה יומי יחיד, נקי ומותאם אישית בלבד.
...
- מאכלים אסורים: ${cleanDisliked || 'אין'}

כללי כמויות קשיחים (מתאימים ל-${calories} קלוריות):
- התאם את כמויות המזון בצלחת באופן נדיב ופרופורציונלי כדי להגיע ל-${calories} קלוריות (למשל: 200-250 גרם חזה עוף/פרגית, 200-250 גרם אורז/פסטה מבושלים, 3 פרוסות לחם מלא, 25-30 גרם שקדים/אגוזים).

חוקי פורמט קשיחים (איסור מוחלט על סוגריים וחישובים):
1. אל תרשום קלוריות או חלבון לצד כל ארוחה! אל תשתמש בסוגריים () בשום מקום.
2. אל תרשום חישובים פנימיים, הערות או הודעות תיקון.
3. אל תשתמש ב-Markdown (ללא **, #, -).

חוקי סינון:
- אסור להשתמש ברכיבים מתוך: "${cleanDisliked}".
- אם צוין "חלב" - אסור להשתמש ביוגורט, גבינות, קוטג' ומעדנים.
- אם צוין "טחינה" - אסור להשתמש בטחינה.
- אם צוין "ביצים" - אסור להשתמש בביצים.

פורמט יציאה קשיח שחובה להעתיק במדויק:
...
💡 טיפ זהב: [טיפ קצר להתמדה]
    ...
    try {
        ...
        menuText = menuText.replace(/#/g, '');
        menuText = menuText.replace(/\(.*?\)/g, ''); // מחיקת סוגריים אוטומטית בשרת
