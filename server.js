...
// --- 🥗 נתיב בניית תפריט תזונה מותאם אישית (FOOLPROOF & EDGE-CASE PROTECTED) ---
app.post('/api/generate-menu', async (req, res) => {
    ...
    const prompt = `
אתה מנוע תזונה קלינית וספורטיבית בכיר. בנה תפריט יומי יחיד ומדויק.
...
- רשימת מאכלים/רגישויות שאסור להשתמש בהם כלל: ${cleanDisliked || 'אין'}

כללי איסור וסינון חמורים (חובה!):
1. עבור על כל המילים ברשימת המאכלים הבלתי רצויים (${cleanDisliked || 'אין'}). אסור באיסור מוחלט להשתמש באף אחד מהם!
2. הרחבת קטגוריות: 
   - אם צוין "חלב"/"מוצרי חלב": חסום גבינות, יוגורט, קוטג', מעדנים.
   - אם צוין "טחינה": חסום טחינה והשתמש בשמן זית/אבוקדו/אגוזים.
   - אם צוין "ביצים": חסום ביצים/חביתה.
   - אם סוננו רוב מקורות החלבון הבשריים/חלביים: השתמש בקטניות (עדשים, חומוס, שעועית), טופו, סייטן, או אגוזים.

כללי ערכים תזונתיים וכמויות חובה:
- 1 פרוסת לחם מלא = 70 קלוריות, 4ג חלבון
- 100ג חזה עוף / פרגית / טונה / בשר / דג = 140 קלוריות, 25ג חלבון
- 100ג אורז / תפו"א / פסטה מבושלים = 120 קלוריות, 2.5ג חלבון
- 10ג שמן זית / טחינה / אגוזים / שקדים = 70 קלוריות
- 1 יוגורט חלבון (200ג) = 140 קלוריות, 20ג חלבון

הנחיה חורצת (איסור מוחלט על תיקונים בטקסט!):
התאם מראש את הגרמים והכמויות בצלחת ב-4 הארוחות כך שסכום הקלוריות והחלבון של כל 4 הארוחות יחד יגיע בדיוק ל-${calories} קלוריות ול-${protein} גרם חלבון!
איסור מוחלט לכתוב "חסר קלוריות", "נוסיף" או "תיקון" בתחתית התפריט.

חוקי הרכבת המנות:
1. ארוחת בוקר: לחם מלא/שיבולת שועל + חלבון מורשה. אל תשים אורז בבוקר!
2. ארוחת צהריים: מנת חלבון מורשת + פחמימה חמה (אורז/תפו"א/פסטה) + ירקות ושמן זית. אל תשים יוגורט בצהריים!
3. ארוחת ביניים: מנת חלבון קלה/פרי + שקדים/אגוזים (בהתאם לסינון).
4. ארוחת ערב: מנת חלבון מורשת + לחם מלא/תפו"א + שומן בריא מורשה + ירקות.
...
💡 טיפ זהב: [טיפ קצר להתמדה]
    ...
    try {
        const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'אתה מנוע תזונה קלינית וספורטיבית מתקדם. תפקידך להפיק תפריט תזונה יחיד, הגיוני, אסתטי ומדויק לחלוטין בשפה העברית תוך ציות מלא לכל אלרגיה או מאכל לא רצוי.'
                    },
                    {
                        role: 'user',
                        content: prompt
        ...
        // 🧹 ניקוי סופי ב-Backend למניעת שגיאות תרגום ועיצוב
        ...
        menuText = menuText.replace(/קוטג' קשה/g, 'גבינה צהובה');
